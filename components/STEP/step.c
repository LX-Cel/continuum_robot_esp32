...
void Read_Sys_Params(uint8_t addr, SysParams_t s)
{
  ...
  if(s >= S_Conf)
  {
    cmd[3] = 0x6B; uart_write_bytes(USART_UX, cmd, 4);
  }
  else
  {
    cmd[2] = 0x6B; uart_write_bytes(USART_UX, cmd, 3);
  }
}
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void Traj_Position_Control(uint8_t addr, uint8_t dir, uint16_t acc, uint16_t dec, float velocity, float position, uint8_t raf, uint8_t snF)
{
...
/**
  ...
  * @retval   地址 + 功能码 + 命令状态 + 校验字节
  */
void Origin_Modify_Params(uint8_t addr, bool svF, uint8_t o_mode, uint8_t o_dir, uint16_t o_vel, uint32_t o_tm, uint16_t sl_vel, uint16_t sl_ma, uint16_t sl_ms, bool potF)
{
  uint8_t cmd[32] = {0};
